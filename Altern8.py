...
class RaceTrackDialog(Frame):
    ...
    def create_widgets(self):
        ...
        self.parent.title("Altern8 Parameters")
        self.grid()

        l1 = Label(self, text="\nPlease enter by how much you want to extend transects: \n(in half track widths) ")
        l1.grid(columnspan=10)

        self.elongation = StringVar(self)
        self.elongation.set(DEFAULTelongation)
        self.sb = Spinbox(self, from_=0, to=10, width = 3, textvariable=self.elongation)
        self.sb.grid( row=0, column= 10, padx=5, pady=5 )

        l2 = Label(self, text="Add transects to complete pattern: ")
...
class RaceTrackDialogxxx(Frame):
    def __init__(self):
        self.parent = Tk()
...
#  finds the appropriate number of waypoints by which to extend a list of zigzag search patterns
def extendPatterns(navwpts, elongation):
    revint = 8  # the number of passes after which the pattern reverses
    ...
    for area in range(len(navwpts) - 1):  # loop through all scan areas but the last one, which is a placeholder
        ...
        if numnavs % 2 == 1:  # if there is an odd number of waypoints in the area
            errorDialog('Scan area %d has odd number of waypoints ' % area)
            return False  # exit the method
        if numnavs < 4:  # if the area contains less than 4 waypoints
            errorDialog('Scan area %d has too few waypoints ' % area)
            return False  # exit the method

        # determine by how much the pattern needs to be extended
        numpasses = numnavs / 2
        if (numpasses % revint == 0) | (numpasses % revint == 1):
            goalpasses = numpasses
        else:
            goalpasses = numpasses - (numpasses % revint) + revint
...
def zigzag2altern8(navwpts, elongation, addtransects):
    revint = 8  # the number of passes after which the pattern reverses

    for area in range(len(navwpts) - 1):  # loop through all scan areas but the last one, which is a placeholder
        # print navwpts
        templist = []
        passorder = [0, 3, 6, 1, 4, 7, 2, 5]
        uu = (len(navwpts[area])/16)*16 / 2   # the number of passes to reorder
        for scanpass in range(uu):
            # determine the correct source pass
            sourcepass = passorder[scanpass % revint] + scanpass - (scanpass % revint)
            templist.append(navwpts[area][sourcepass * 2])
            templist.append(navwpts[area][sourcepass * 2 + 1])
            # print templist
        for scanpass in range( uu, len(navwpts[area]) /2 ):
            templist.append(navwpts[area][scanpass * 2])
            templist.append(navwpts[area][scanpass * 2 + 1])
        navwpts[area] = templist

# lengthen the track, so turning occurs outside of the scan area
def lengthenTrack(navwpts, elongation, addtransects):
    revint = 8  # the number of passes after which the pattern reverses
    for area in range(len(navwpts) - 1):  # loop through all scan areas but the last one, which is a placeholder
        ...
        offset = vectScalar(dirP, dist * elongation / 2.0)
        uu = len(navwpts[area])
        for wpt in range(uu):
            if ((wpt + 1) / 2) % 2 == 0:
                ...
                navwpts[area][wpt][8] = '{:-.6f}'.format(float(navwpts[area][wpt][8]) + offset[0])
                navwpts[area][wpt][9] = '{:-.6f}'.format(float(navwpts[area][wpt][9]) + offset[1])
        # doubly expand transects that cannot be reordered
        offset = vectScalar(dirP, dist * elongation / 2.0)
        for wpt in range(uu -(uu %(revint *2)), uu):
            if ((wpt + 1) / 2) % 2 == 0:
                navwpts[area][wpt][8] = '{:-.6f}'.format(float(navwpts[area][wpt][8]) - offset[0])
                navwpts[area][wpt][9] = '{:-.6f}'.format(float(navwpts[area][wpt][9]) - offset[1])
            else:
        ...
        if mission[i][3] == '206':  # check if this line turns the camera on or off
            ...
            if mission[i][3] == '16':  # if this command is a nav waypoint
                ...
                if len(navwpts[area]) == 1:  # but we have not yet reached the 2nd wpt in the scan area
                    ...
                    infoDialog('Unexpected command in wpt ' + mission[i][
...
#sourcefilename = 'C:\\Users\\aanderson29\\Documents\\CITRIS\Missions\\OD\\Top_Long4.txt'
...
elongation = params.returnval[0]
addtransects = params.returnval[1]
inbetween = []  # everything that is not a transect
navwpts = []    # transect waypoints
anomalies = []  # non-waypoint commands within scanning area
s = []
...
if addtransects == True:
    if extendPatterns(navwpts, elongation) == False:  # add transects to pattern
        sys.exit()                                    # exit if something goes wrong
# print 'scan areas extended'

# move waypoints outward to make sure the plane flies straight over the scan area
if elongation != 0:
    lengthenTrack(navwpts, elongation, addtransects)

# shuffle waypoints to produce alternating pattern with wide turns
zigzag2altern8(navwpts, elongation, addtransects)
...
filename = filenameblocks[0] + "_Altern8.txt"
f = open(filename, "w")
f.write(line)
f.close()
